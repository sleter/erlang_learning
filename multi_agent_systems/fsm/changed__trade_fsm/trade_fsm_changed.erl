...
%% additional functions for excersise purposes
-export([wait_for_reply/2, force_reply/2, force_reply/3, wait_for_reply/3]).
...
idle({negotiate, OtherAddress={RegisteredName, NodeName}}, From, S=#state{}) ->
    OtherPid = rpc:call(NodeName, erlang, whereis, [RegisteredName]),
...
negotiate({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
    ...
    {next_state, wait_for_reply, S#state{ownitems=add(Item, OwnItems)}};
%% Own side retracting an item offer
negotiate({retract_offer, Item}, S=#state{ownitems=OwnItems}) ->
    undo_offer(S#state.other, Item),
    notice(S, "cancelling offer on ~p", [Item]),
    {next_state, wait_for_reply, S#state{ownitems=remove(Item, OwnItems)}};
% %% other side offering an item
negotiate({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "other player offering ~p", [Item]),
    {next_state, force_reply, S#state{otheritems=add(Item, OtherItems)}};
%% other side retracting an item offer
negotiate({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "Other player cancelling offer on ~p", [Item]),
    {next_state, force_reply, S#state{otheritems=remove(Item, OtherItems)}};
...
wait_for_reply({do_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "other player offering ~p", [Item]),
    {next_state, negotiate, S#state{otheritems=add(Item, OtherItems)}};
wait_for_reply({undo_offer, Item}, S=#state{otheritems=OtherItems}) ->
    notice(S, "Other player cancelling offer on ~p", [Item]),
...
force_reply({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
    do_offer(S#state.other, Item),
    notice(S, "offering ~p", [Item]),
    {next_state, negotiate, S#state{ownitems=add(Item, OwnItems)}};
force_reply({retract_offer, Item}, S=#state{ownitems=OwnItems}) ->
    undo_offer(S#state.other, Item),
    notice(S, "cancelling offer on ~p", [Item]),
